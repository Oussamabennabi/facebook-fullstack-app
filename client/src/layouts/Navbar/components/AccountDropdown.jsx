...
import {DisplayDropdown, HelpDropdown, KeyboardDropdown, MainDropdown, SelectProfileDropdown, SettingsDropdown} from './dropdowns';
...
export const AccountDropdown = () => {
  ...
  return (
    <div className="dropdown" style={{ height: menuHeight }}>
      ...
      >
        <div className="menu">
          <SelectProfileDropdown setActiveMenu={setActiveMenu} />
        </div>
      </CSSTransition>
    </div>
  );
...
export const DropdownItem = ({ children, leftIcon, rightIcon, isRadio }) => {
  return (
    <div className="dropdown-item">
      <span className="left-icon">{leftIcon}</span>
      {children}
      <span className={` ${!isRadio&&"right-icon"}`}>{rightIcon}</span>
    </div>
  );
};
